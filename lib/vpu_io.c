...
vpu_mem_desc code_buf = { 0 };
vpu_mem_desc para_buf = { 0 };
...
static int get_system_rev(void)
{
	...
	/*
	 * The Quby R10-h28 kernel reports "Revision : 0000", so system_rev
	 * carries no usable SoC id (mxc_cpu() would be 0 and cpu_is_mx27()
	 * false, making firmware selection fall through to a bogus filename).
	 * This board is an i.MX27, so force a valid rev. TO2 (0x27020) is the
	 * common silicon; the driver logs the real SYSCTRL silicon_rev at init
	 * -- switch to 0x27010 if it reports TO1 (silicon_rev 0).
	 */
	if ((system_rev >> 12) != 0x27) {
		system_rev = 0x27020;
		ret = 0;
	}
...
int IOSystemInit(void *callback)
{
	...
	/* Allocate work buffer (pre-allocated by driver, so this returns
	 * the held buffer without a new dma_alloc_coherent call) */
	bit_work_addr.size = WORK_BUF_SIZE;
	if (_IOGetPhyMem(VPU_IOC_GET_WORK_ADDR, &bit_work_addr) < 0) {
		err_msg("Get work buffer address failed!\n");
		goto err;
	}
	if (IOGetVirtMem(&bit_work_addr) <= 0)
		goto err;

	/* Allocate code buffer separately (64KB, order 4) */
	code_buf.size = CODE_BUF_SIZE;
	if (_IOGetPhyMem(VPU_IOC_PHYMEM_ALLOC, &code_buf) < 0) {
		err_msg("Get code buffer address failed!\n");
		goto err2;
	}
	if (IOGetVirtMem(&code_buf) <= 0)
		goto err2;

	/* Allocate para+para2 buffer separately (~12KB, order 2) */
	para_buf.size = PARA_BUF2_SIZE + PARA_BUF_SIZE;
	if (_IOGetPhyMem(VPU_IOC_PHYMEM_ALLOC, &para_buf) < 0) {
		err_msg("Get para buffer address failed!\n");
		goto err3;
	}
	if (IOGetVirtMem(&para_buf) <= 0)
		goto err3;

	info_msg("VPU bufs: work phy=0x%lx virt=0x%lx | code phy=0x%lx virt=0x%lx | "
		 "para phy=0x%lx virt=0x%lx | reg_base=0x%lx\n",
		 bit_work_addr.phy_addr, bit_work_addr.virt_uaddr,
		 code_buf.phy_addr, code_buf.virt_uaddr,
		 para_buf.phy_addr, para_buf.virt_uaddr, (unsigned long)vpu_reg_base);

	UnlockVpu(vpu_semap);
	return 0;

      err3:
	IOFreeVirtMem(&code_buf);
	IOFreePhyMem(&code_buf);
      err2:
...
unsigned long VpuWriteReg(unsigned long addr, unsigned int data)
{
	...
	rw.offset = (unsigned int)addr;
	rw.value = data;
	/* Registers are written in the kernel: userspace can't access the
	 * i.MX27 peripheral bus directly (non-privileged accesses abort). */
	if (ioctl(vpu_fd, VPU_IOC_REG_WRITE, &rw) < 0)
		err_msg("VpuWriteReg(0x%lx) failed\n", addr);
...
unsigned long VpuReadReg(unsigned long addr)
{
	...
	rw.offset = (unsigned int)addr;
	rw.value = 0;
	if (ioctl(vpu_fd, VPU_IOC_REG_READ, &rw) < 0)
		err_msg("VpuReadReg(0x%lx) failed\n", addr);
	return rw.value;
}
...
void ResetVpu(void)
{
	/* Register access must go through the kernel (userspace can't reach the
	 * i.MX27 peripheral bus). This was a direct *reg_map() deref -> abort. */
	unsigned long val = VpuReadReg(BIT_CODE_RESET);
	VpuWriteReg(BIT_CODE_RESET, val | 0x1);
	usleep(10);
	VpuWriteReg(BIT_CODE_RESET, val & ~0x1);
