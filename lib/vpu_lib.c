...
/* bit_work_addr and code_para_buf are declared in vpu_io.h */
...
RetCode vpu_Init(void *cb)
{
	...
	codeBuffer = code_buf.phy_addr;
	workBuffer = bit_work_addr.phy_addr;
	paraBuffer = para_buf.phy_addr + PARA_BUF2_SIZE;

	virt_codeBuf = (Uint32) (code_buf.virt_uaddr);
	virt_paraBuf = (unsigned long *)(para_buf.virt_uaddr + PARA_BUF2_SIZE);
	virt_paraBuf2 = (unsigned long *)(para_buf.virt_uaddr);
...
RetCode vpu_DecGetOutputInfo(DecHandle handle, DecOutputInfo * info)
{
	...
	paraBuffer =
			    para_buf.phy_addr + PARA_BUF2_SIZE;
	ret = CheckDecInstanceValidity(handle);
	if (ret != RETCODE_SUCCESS)
		return ret;
