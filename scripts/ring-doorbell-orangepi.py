#!/usr/bin/env python3
"""
Ring to Toon Streamer
---------------------
An automated event-driven service designed for Orange Pi 5 (Ubuntu) to receive
motion/person detection events from `ring-mqtt` via MQTT, and automatically transcode
the stream and send it to a Toon 1 thermostat over RTP.
"""
...
TOON_PORT = 5000          # RTP destination port for the video stream
...
def run_ffmpeg(rtsp_url: str):
    cmd = [
        ...
        "-loglevel", "warning",
        "-nostdin",
        "-hwaccel", "rkmpp",                  # Hardware accelerated MPP decoder
        ...
        # RK3588 optimized RGA scale filter (nv12 directly transcoded using hardware rasterizer)
